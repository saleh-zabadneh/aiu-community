...
export const SignupFormGrid = () => {
  ...
  return (
    <div className="py-20 bg-zinc-950 text-zinc-200 selection:bg-zinc-600">
      {/* <BubbleButton className="absolute text-sm left-4 top-6">
        <FiArrowLeft />
        Go back
      </BubbleButton> */}
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-xl p-4 mx-auto"
      >
        <Heading />
...
const SocialOptions = () => (
  <div>
    <div className="flex gap-3 mb-3">
      <BubbleButton className="flex justify-center w-full py-3">
        <SiTwitter />
      </BubbleButton>
      <BubbleButton className="flex justify-center w-full py-3">
        <SiGithub />
...
const SignUpForm = () => {
  const navigate = useNavigate();
  const { login } = useAuth();
  const { isCreating, createFunction } = useCreate("user", "/user/signup", {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-3">
        <label htmlFor="student_id" className="mb-1.5 block text-zinc-400">
          Student Id
        </label>
        <input
          disabled={isCreating}
          ...
          placeholder="20*******"
          required
          className="w-full px-3 py-2 transition-shadow border rounded-md border-zinc-700 bg-zinc-900 placeholder-zinc-500 ring-1 ring-transparent focus:outline-0 focus:ring-blue-700"
        />
        {errors.student_id && <p>{errors.student_id.message}</p>}
      </div>
      <div className="mb-3">
        <label htmlFor="full_name" className="mb-1.5 block text-zinc-400">
          Full Name
        </label>
        <input
          disabled={isCreating}
      ...
      <div className="mb-6">
        <label htmlFor="password_confirmation" className="block text-zinc-400">
          Confirm Password
        </label>
        <input
