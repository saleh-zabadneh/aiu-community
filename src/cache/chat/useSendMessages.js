...
import { apiSendMessage } from "@/services/chat/apiSendMessages";

export function useSendMessages(keyName, url, toastMessage) {
  const { user } = useAuth();
  ...
  const { mutate: createFunction } = useMutation({
    mutationFn: (data) =>
      apiSendMessage(user ? user.token : "no token", url, data),
    onSuccess: () => {
